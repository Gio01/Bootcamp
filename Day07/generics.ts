...
let numberList = new List<number>();//should be restricted to only handle numbers
let stringList = new List<string>();//should be restricted to only handle strings

let nos = new Array<number>();
/**
 * so here we can actually specify the type of array that we want and the 
 * way this is possible is that by default the Array class is a generic which 
 * means we can speficy the type that we want when creating a variable!!
 */

//Generic function
//this function will take in a list and then return a new list
/**
 * here we need to create a list based on the type of item to then create a 
 * list of that type
 */

//we can also create a type definition for this generic addItem function!
type addItemFnType<T> = (list: Array<T>, item: T) => Array<T>
...
function myfind<T>(list: Array<T>, predicate: (item: T) => boolean ): T | undefined{
...
function myfilter<T>(list: Array<T>, predicate: (item: T) => boolean ): Array<T>{
...
let predicate = function(item:any) {
    //console.log(item)
...
function map<T, T2>(sourceList: Array<T>, transformationFn: (item: T) => T2): Array<T2>{
//the types does not have to be T you can actually write srcType and targetType
    let result = new Array<T2>;
...
function transformationFn(item:any) {
    if(item % 2 == 0){
        return 'even'
    }
    else{
...
function transformationFn2(item:any) {
    return item * 10
}
...
//type constraint 
//we will use the extends keyword to type constraint!

// function add<T extends number | string> (x: T, y: T): void{
//     /**
//      * The reason why we get an error of: Operator '+' cannot be applied to types 'T' and 'T
//      * is because using t + will not work for all types!
//      * 
//      * it will however work with numbers or strings
//      */
...
function sortByComparer<T>(list: Array<T>, comparer: Comparer<T>): void {
...
function sortByAttributeName<T, Key extends keyof T> (list: Array<T>, attrName: Key){
...
function printProps<T, Key extends keyof T>(item: T, listAttr: Array<Key>): void{
