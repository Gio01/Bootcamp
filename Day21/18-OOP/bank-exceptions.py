...
class CheckingAccount(BankAccount): #Here CheckingAccount is Inheriting from the BankAccount class
    ...
    def withdraw(self, amount):
        ...
        protected attributes is only a convetion to tell us that we should not be able to access
        them within an instance of the class but this is a convention only to tell other 
        developers of how they should be treated when in fact that are still accessible within 
        the language! 
